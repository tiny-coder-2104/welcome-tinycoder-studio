...
export function validateDraft(raw) {
  ...
  // 0062: ₱ + pesos added — old /\$\s?\d/ passed PHP prices after currency switch
  if (/[$₱]\s?\d/.test(t) || /\b\d+\s?(usd|php|eur|pesos?)\b/i.test(t)) {
...
if (typeof process !== 'undefined' && import.meta.url === 'file://' + process.argv[1]) {
  ...
  assert(validateDraft('The starter package is ₱25,000 total') === null, '₱25,000 rejected');
  assert(validateDraft('It will be about 15000 pesos all-in') === null, '15000 pesos rejected');
  ...
  console.log('lib/suggest.js demo: all 18 checks passed.');
}
